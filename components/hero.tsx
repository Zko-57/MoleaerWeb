...
const SCROLL_SEG: [number, number] = [0, 1];
const PARALLAX_CONTENT: [number, number] = [0, 36];
const PARALLAX_OFF: [number, number] = [0, 0];
const PARALLAX_VISUAL: [number, number] = [0, -24];
...
export function Hero() {
  ...
  const visualScale = useTransform(scrollYProgress, SCROLL_SEG, parallax ? PARALLAX_SCALE_ON : PARALLAX_SCALE_OFF);

  return (
    <section
      ref={sectionRef}
      className="relative mx-auto flex max-w-[1320px] flex-col gap-10 px-5 pb-[var(--section-y)] pt-28 sm:px-8 sm:pt-32 lg:flex-row lg:items-start lg:gap-14 lg:pt-40"
    >
      <m.div
        className="flex-1 space-y-6 pt-2 sm:space-y-7 sm:pt-3 lg:pt-2"
      ...
      >
        ...
        <RevealStaggerRow items={HERO_STAGGER_ITEMS} />
        <Reveal delay={0.22}>
